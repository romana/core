...
func (h Helper) ensureRouteToEndpoint(netif *NetIf) error {
	mask := fmt.Sprintf("%d", h.Agent.config.EndpointNetmask())
...
func (h Helper) otherHosts() []config.PocHost {
	index := h.Agent.config.CurrentHostIndex
...
func (h Helper) ensureInterHostRoutes() error {
	...
	for j := range OtherHosts {
		...
		log.Print("Acquiring mutex ensureInterhosRoutes")
		h.ensureInterHostRoutesMutex.Lock()
		defer func() {
			log.Print("Releasing mutex ensureInterhosRoutes")
			h.ensureInterHostRoutesMutex.Unlock()
		}()
		log.Print("Acquired mutex ensureInterhosRoutes")
...
func (h Helper) waitForIface(expectedIface string) bool {
	for i := 0; i <= h.Agent.config.waitForIfaceTry; i++ {
